<?php
require("./header.php");
?>
<h1>Tareas de clase</h1>
<ul>
	<li><a href="./ELEMENTOS_DE_SEGURIDAD">SEGURIDAD<a></li>
	<li><a href="./Examenes">Examenes Curso 2019/2020<a></li>
</ul>
<ol>
	<li><a href="./Tema1">Tema1<a></li>
	<li><a href="./Tema2">Tema2<a></li>
	<li><a href="./Tema3">Tema3<a></li>
	<li><a href="./Tema4">Tema4<a></li>
	<li><a href="./Tema5">Tema5<a></li>
	<li><a href="./Tema6">Tema6<a></li>
	<li><a href="./Tema7">Tema7<a></li>
	<li><a href="./Tema8">Tema8<a></li>
	<li><a href="./Tema9">Tema9<a></li>
	<li><a href="./Tema10">Tema10<a></li>
	<li><a href="./Tema11">Tema11<a></li>
	<br>
	<form class="" action="index.html" method="post">
			<button type="submit" name="Atras" value="volver" formaction="http://localhost/">Volver</button>
	</form>

</ol>
<a href="phpInfo.php">Información sobre PHP</a>
<?php
include("footer.php");
?>
